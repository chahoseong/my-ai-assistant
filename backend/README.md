...
- `weather`와 `opgg_tft`는 각각 독립된 수명주기로 시작합니다. 하나가 시작에 실패해도
  API와 다른 도구는 계속 사용할 수 있습니다.
- OP.GG의 원문 MCP schema와 결과는 모델에 직접 전달하지 않습니다. 모델은 구조를
  확인하는 `tft_describe_meta_decks`와 제한된 조회를 수행하는 `tft_query_meta_decks`만
  사용합니다.
