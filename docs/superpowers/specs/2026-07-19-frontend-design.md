...
화면용 대화 상태는 서버의 `Conversation` 응답과 분리해 App이 소유한다.
`status`는 `created`(새 대화의 첫 메시지 흐름), `displayed`(사이드바에서
선택한 대화), `hidden`(현재 보이지 않는 대화) 중 하나이며, 응답 생성 여부는
`isStreaming` 부울로 표현한다. `created` 상태에서는 첫 메시지의 낙관적 화면과
스트림을 사용하므로 선택 효과의 일반 히스토리 조회를 생략한다. 스트림이 끝나도
`created`는 유지하며, 사용자가 다른 대화를 선택할 때 `hidden`으로 전환한다.

## 브라우저와 보안 계약

Vite는 `http://127.0.0.1:5173`에서 실행하며 `/api`를
`http://127.0.0.1:8001`으로 프록시한다. 요청은
...
완화가 아니다. `http://localhost:5173`은 지원하지 않으며, UI는 모든 메시지 내용을 HTML이 아닌 일반 React 텍스트로
렌더링한다.
...
3. 목록: `GET /api/conversations`가 최신순 사이드바를 제공한다. 초기 목록의
   대화는 `hidden` 상태로 등록하며, 제목이 null이면 생성 시각을 표시한다.
4. 읽기: `displayed` 대화를 선택하면 `GET /api/conversations/{id}/messages`를
   호출한다. `created` 대화는 첫 스트림이 끝날 때까지 선택 효과의 자동 조회를
   생략하고, 종료 후 저장 상태를 명시적으로 동기화한다.
...
6. 새 대화: “새 대화” 클릭은 UI 상태만 바꾼다. 첫 번째 공백이 아닌 메시지가
   앞 30개 Unicode 코드 포인트를 제목으로 사용해 대화를 만든 뒤, 즉시
   `created` 상태로 표시하고 같은 메시지를 스트리밍 전송한다. 스트림 완료 또는
   SSE 오류 뒤에는 저장 상태를 동기화하지만 `created` 상태는 유지한다.
...
작성창은 Enter로 전송하고 Shift + Enter로 줄바꿈하며, IME 조합 중인 Enter는
전송하지 않는다.
